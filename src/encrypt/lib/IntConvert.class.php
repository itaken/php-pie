...
final class IntConvert
{
    ...
    {
        return self::ENCODE_MAP[hexdec($hash)];
    }
	 ...
	 * @return string
     */
    private static function fixBin($bin = '110')
    {
        $len = strlen($bin);
        if ($len % 5 != 0) {
	 ...
	 * @return string
     */
    private static function getHash($num = 0)
    {
    ...
    public static function randomKey()
    {
        ...
        echo "  const ENCODE_MAP = [\n";

        for ($i = 0; $i < 16; $i++) {
            $keys = self::ENCODE_MAP[0];
            $keys_new = '';
            $word = '';
	 ...
	 * @return string
     */
    public static function toString($num = 0)
    {
        // 对传入的数字，取hash的第一位
