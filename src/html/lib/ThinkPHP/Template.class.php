...
class Template
{
    ...
    public function __construct($template_filename = '', $config=[])
    {
        if (!empty($config['compiled'])) {
            $this->temp_dir = $config['compiled'];
        }
        if (!empty($config['cache'])) {
            $this->cache_dir = $config['cache'];
        }
        if (is_numeric($config['cache_lifetime'])) {
            $this->cache_lifetime = $config['cache_lifetime'];
        }
        if (!empty($config['template_dir']) && is_file($config['template_dir'] . '/' . $template_filename)) {
        ...
        if (preg_match_all('/{([a-zA-Z0-9_]+):([^}]*)}/', $page, $var)) {
            foreach ($var[2] as $cnt => $tag) {
                ...
                if (!isset($this->extension_tagged[$extension])) {
                    $header .= "include_once \"" . str_replace("\\", "/", dirname(__FILE__)) . "/templateExtensions/" . $extension . ".php\";\n";
